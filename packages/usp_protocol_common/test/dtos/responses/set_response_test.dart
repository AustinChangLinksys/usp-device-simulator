...
void main() {
  group('UspSetResponse', () {
    test('should correctly assign properties', () {
      ...
      final failures = {
        UspPath.parse('Device.WiFi.Password'): UspException(1, 'Invalid password'),
      };
      final response = UspSetResponse(successPaths: success, failurePaths: failures);
    ...
    test('should support value equality', () {
      ...
      final response1 = UspSetResponse(successPaths: success1, failurePaths: failures1);

      final success2 = [UspPath.parse('Device.WiFi.SSID')];
      final failures2 = {
        UspPath.parse('Device.WiFi.Password'): UspException(1, 'Invalid password'),
      };
      final response2 = UspSetResponse(successPaths: success2, failurePaths: failures2);

      final success3 = [UspPath.parse('Device.WiFi.Alias')];
      final failures3 = {
        UspPath.parse('Device.WiFi.SomethingElse'): UspException(2, 'Error'),
      };
      final response3 = UspSetResponse(successPaths: success3, failurePaths: failures3);
